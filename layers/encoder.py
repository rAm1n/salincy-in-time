...
class Encoder(nn.Module):
	...
	def __init__(self, features):
		...
		self.embedding = nn.Sequential(
			nn.Conv2d(896, 32, kernel_size=(3,3), stride=1, padding=1),
	...
	def forward(self, x, extracted_layers=['9', '16', '23']):
		...
		# merge = torch.cat([self.avp2(outputs[0]), outputs[1], self.up2(outputs[2])], dim=1)
...
def make_encoder(pretrained=False, config=[64, 64, 'M', 128, 128, 'M', 256, 256, 256, 'M', 512, 512, 512, 'M', 512, 512, 512, 'M'], **kwargs):
